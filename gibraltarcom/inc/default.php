...
function seo_warning() {
	...
	$message = __( 'You are blocking access to robots. You must go to your <a href="%s">Reading</a> settings and uncheck the box for Search Engine Visibility.', 'gibraltarcom' );
...
function theme_localization () {
	load_theme_textdomain( 'gibraltarcom', get_template_directory() . '/languages' );
}
...
function wp_admin_bar_valid_search_menu( $wp_admin_bar ) {
	...
	$form .= '<input type="submit" class="adminbar-button" value="' . __( 'Search', 'gibraltarcom' ) . '"/>';
	$form .= '</div></form>';
...
function theme_get_the_password_form() {
	...
	<p>' . __( 'This content is password protected. To view it please enter your password below:', 'gibraltarcom' ) . '</p>
	<p><label for="' . $label . '">' . __( 'Password:', 'gibraltarcom' ) . '</label> <input name="post_password" id="' . $label . '" type="password" size="20" /> <input type="submit" name="Submit" value="' . esc_attr__( 'Submit', 'gibraltarcom' ) . '" /></p></form>
	';
	return $output;
}
...
add_action( 'admin_init',
	function (){
		add_settings_section(
			'eg_setting_section',
			__( 'Date archive link', 'gibraltarcom' ),
			function () {},
			'reading'
		);
...
function eg_setting_callback_function(){
	...
	echo '
	 <select name="eg_date_archive_link_type">
		 <option ' . selected( $type, 'day', false ) . ' value="day">' . __( 'Day', 'gibraltarcom' ).'</option>
		 <option ' . selected( $type, 'month', false ) . ' value="month">' . __( 'Month', 'gibraltarcom' ).'</option>
		 <option ' . selected( $type, 'year', false ) . ' value="year">' . __( 'Year', 'gibraltarcom' ).'</option>
	 </select>
	';
}
