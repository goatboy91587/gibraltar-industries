	...
	grsNotify.prototype._initButtons = function() {
		for(var i = 0; i < this._data.btns.length; i++) {
			if(this.getOpt('btns', this._data.btns[ i ]+ '_enb')) {
				var self = this;
				this._$.find('#grsBtn_'+ self._data.btns[ i ]).data('grs_btn', self._data.btns[ i ]).click(function(e){
					e.preventDefault();
					
					var switchOn = jQuery(this).data('grs_btn');
					if (typeof switchOn === 'undefined') {
						switchOn = jQuery(this).attr('id').replace('grsBtn_', '');
					}
