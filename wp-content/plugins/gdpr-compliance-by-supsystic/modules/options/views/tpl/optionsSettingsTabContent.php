...
<section>
	<form id="ppsSettingsForm" class="ppsInputsWithDescrForm">
						...
						<?php if(isset($optCatData['opts']) && !empty($optCatData['opts'])) { ?>
							<?php foreach($optCatData['opts'] as $optKey => $opt) { ?>
								...
								>
									...
									<td class="col-w-1perc">
										<?php echo htmlGrs::$htmlType('opt_values['. $optKey. ']', $htmlOpts)?>
									</td>
		...
		<?php echo htmlGrs::hidden('mod', array('value' => 'options'))?>
		<?php echo htmlGrs::hidden('action', array('value' => 'saveGroup'))?>
	</form>
</section>