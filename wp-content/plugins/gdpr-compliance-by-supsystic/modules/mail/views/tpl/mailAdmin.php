<form id="ppsMailTestForm">
	<label>
		...
		<?php echo htmlGrs::text('test_email', array('value' => $this->testEmail))?>
	</label>
	<?php echo htmlGrs::hidden('mod', array('value' => 'mail'))?>
	<?php echo htmlGrs::hidden('action', array('value' => 'testEmail'))?>
	<button class="button button-primary">
		<i class="fa fa-paper-plane"></i>
		<?php _e('Send test', GRS_LANG_CODE)?>
	</button><br />
	<i><?php _e('This option allows you to check your server mail functionality', GRS_LANG_CODE)?></i>
</form>
<div id="ppsMailTestResShell" style="display: none;">
	<?php _e('Did you receive test email?', GRS_LANG_CODE)?><br />
	<button class="ppsMailTestResBtn button button-primary" data-res="1">
		<i class="fa fa-check-square-o"></i>
		<?php _e('Yes! It works!', GRS_LANG_CODE)?>
	</button>
	<button class="ppsMailTestResBtn button button-primary" data-res="0">
		<i class="fa fa-exclamation-triangle"></i>
		<?php _e('No, I need to contact my hosting provider with mail function issue.', GRS_LANG_CODE)?>
	</button>
</div>
<div id="ppsMailTestResSuccess" style="display: none;">
	<?php _e('Great! Mail function was tested and is working fine.', GRS_LANG_CODE)?>
</div>
<div id="ppsMailTestResFail" style="display: none;">
	<?php _e('Bad, please contact your hosting provider and ask them to setup mail functionality on your server.', GRS_LANG_CODE)?>
</div>
...
<form id="ppsMailSettingsForm">
		...
		<?php foreach($this->options as $optKey => $opt) { ?>
			...
			<tr>
				...
				<td class="col-w-1perc">
					<?php echo htmlGrs::$htmlType('opt_values['. $optKey. ']', array('value' => $opt['value'], 'attrs' => 'data-optkey="'. $optKey. '"'))?>
				</td>
	...
	<?php echo htmlGrs::hidden('mod', array('value' => 'mail'))?>
	<?php echo htmlGrs::hidden('action', array('value' => 'saveOptions'))?>
	<button class="button button-primary">
		<i class="fa fa-fw fa-save"></i>
		<?php _e('Save', GRS_LANG_CODE)?>
	</button>
</form>
