...
jQuery.fn.serializeAnythingGrs = function(addData, returnArray) {
...
function grs_str_replace(haystack, needle, replacement) { 
	var temp = haystack.split(needle); 
	return temp.join(replacement); 
}
/**
...
function toeListableGrs(params) {
	...
	this.draw = function(list, count) {
		this.table.find('tr').not('.grsExample, .grsTblHeader').remove();
		var exampleRow = this.table.find('.grsExample');
		for(var i in list) {
			...
			newRow.removeClass('grsExample').show();
...
function setCookieGrs(c_name, value, exdays) {
	var exdate = new Date();
 ...
 * @returns {jQuery}
 */
jQuery.fn.grsZoom = function(level, origin) {
	origin = origin ? origin : 'center center';
	jQuery(this).data('zoom', level);
	return jQuery(this).css({
	/*	'zoom': level	// Didn't worked correctly for mobiles
...
/**
 * Retrive worgrsess attach ID from image, using img classes
...
function randGrs( min, max, returnFloat ) {
