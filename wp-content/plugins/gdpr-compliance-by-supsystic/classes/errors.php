...
class errorsGrs {
    ...
    static public function setSession($error) {
        ...
        if(is_array($error))
            ...
            $sesErrors[] = $error;
        reqGrs::setVar('sesErrors', $sesErrors, 'session');
    }
    static public function init() {
        $grsErrors = reqGrs::getVar('grsErrors');
        if(!empty($grsErrors)) {
            if(!is_array($grsErrors)) {
                $grsErrors = array( $grsErrors );
            }
            $grsErrors = array_map('htmlspecialchars', array_map('stripslashes', array_map('trim', $grsErrors)));
            if(!empty($grsErrors)) {
                self::$current = $grsErrors;
				if(is_admin()) {
					add_action('admin_notices', array('errorsGrs', 'showAdminErrors'));
				} else {
					add_filter('the_content', array('errorsGrs', 'appendErrorsContent'), 99999);
				}
            }
        }
    }
    ...
    static public function getSession() {
        return reqGrs::getVar('sesErrors', 'session');
    }
    static public function clearSession() {
        reqGrs::clearVar('sesErrors', 'session');
    }
