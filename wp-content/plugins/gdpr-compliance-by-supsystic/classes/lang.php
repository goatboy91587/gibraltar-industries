...
class langGrs {
	...
	static public function extract($d = array('dir' => '', 'langGrs' => '')) {
		$data = array();
		if(isset($d['dir']) && !empty($d['dir']))
			$langDirPath = $d['dir'];
		else if(isset($d['langGrs']) && !empty($d['langGrs']))
			$langDirPath = GRS_LANG_DIR. $d['langGrs']. DS;
		else
			$langDirPath = GRS_LANG_DIR. GRS_WPLANG. DS;
