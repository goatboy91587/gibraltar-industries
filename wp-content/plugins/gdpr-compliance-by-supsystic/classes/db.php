...
class dbGrs {
     ...
     * #__  Worgrsess prefix
     * ^__  Store plugin tables prefix (@see GRS_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, GRS_DB_PREF, $wpdb->prefix. GRS_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampGrs($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, GRS_DATE_DL)) return dateToTimestampGrs($date);
        $arr = explode('-', $date);
        return dateToTimestampGrs($arr[2]. GRS_DATE_DL. $arr[1]. GRS_DATE_DL. $arr[0]);
    }
