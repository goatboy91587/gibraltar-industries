...
abstract class controllerGrs {
	...
	protected function _createModel($name = '') {
		if(empty($name)) $name = $this->getCode();
		$parentModule = frameGrs::_()->getModule( $this->getCode() );
		$className = '';
		if(importGrs($parentModule->getModDir(). 'models'. DS. $name. '.php')) {
			$className = toeGetClassNameGrs($name. 'Model');
		}
	...
	protected function _createView($name = '') {
		...
		if(importGrs($parentModule->getModDir(). 'views'. DS. $name. '.php')) {
			$className = toeGetClassNameGrs($name. 'View');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(GRS_ADMIN),
				'remove' => array(GRS_ADMIN),
				'restore' => GRS_ADMIN,
			),
			S_USERLEVELS => array(
	 ...
	 * Can be used on of sub-array - GRS_METHODS or GRS_USERLEVELS
	 */
	...
	public function getModule() {
		return frameGrs::_()->getModule( $this->getCode() );
	}
	...
	public function getListForTbl() {
		...
		$page = (int) reqGrs::getVar('page');
		$rowsLimit = (int) reqGrs::getVar('rows');
		$orderBy = reqGrs::getVar('sidx');
		$sortOrder = reqGrs::getVar('sord');

		// Our custom search
		$search = reqGrs::getVar('search');
		...
		// jqGrid search
		$isSearch = reqGrs::getVar('_search');
		if($isSearch) {
			$searchField = trim(reqGrs::getVar('searchField'));
			$searchString = trim(reqGrs::getVar('searchString'));
		...
		$res = dispatcherGrs::applyFilters($this->getCode(). '_getListForTblResults', $res);
		$res->ajaxExec();
	}
	public function removeGroup() {
		$res = new responseGrs();
		if($this->getModel()->removeGroup(reqGrs::getVar('listIds', 'post'))) {
