 ...
 */
function getRandNameGrs($lenFrom = 6, $lenTo = 9) {
	$res = '';
...
function setDefaultParamsGrs($params, $default) {
	foreach($default as $k => $v) {
		$params[$k] = isset($params[$k]) ? $params[$k] : $default[$k];
	}
	return $params;
}
function importClassGrs($class, $path = '') {
	if(!class_exists($class)) {
		if(!$path) {
			$classFile = $class;
			if(strpos(strtolower($classFile), GRS_CODE) !== false) {
				$classFile = preg_replace('/'. GRS_CODE. '/i', '', $classFile);
			}
			$path = GRS_CLASSES_DIR. $classFile. '.php';
		}
		return importGrs($path);
		...
		/*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
			...
			if(strpos($classFile, GRS_DIR) === false) {   //Class is not in our plugin directory
				...
				if(!empty($plugins)) {
					for($i = 0; $i < count($plugins); $i++) {
						if(strpos($plugins[$i], GRS_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
							unset($plugins[$i]);
						}
					}
 ...
 */
function toeGetClassNameGrs($class) {
	$className = '';
	if(class_exists($class. strFirstUpGrs(GRS_CODE))) 
		$className = $class. strFirstUpGrs(GRS_CODE);
	else if(class_exists(GRS_CLASS_PREFIX. $class)) 
		$className = GRS_CLASS_PREFIX. $class;
	else
		$className = $class;
	return $className;
}
 ...
 */
function toeCreateObjGrs($class, $params) {
	$className = toeGetClassNameGrs($class);
	$obj = NULL;
...
function in_array_arrayGrs($needle, $haystack) {
	if(is_array($needle)) {
		foreach($needle as $n) {
		...
		return in_array_arrayGrs($needle, $haystack);
}
function json_encode_utf_normalGrs($value) {
	if (is_int($value)) {
		return (string)$value;   
	...
	if ($with_keys) {
		foreach ($value as $key => $v) {
			$result[] = json_encode_utf_normalGrs((string)$key) . ':' . json_encode_utf_normalGrs($v);    
		}
		return '{' . implode(',', $result) . '}';                
	} else {
		foreach ($value as $key => $v) {
			$result[] = json_encode_utf_normalGrs($v);    
		}
		return '[' . implode(',', $result) . ']';
	}
}
 ...
 * @return array
 */
function prepareParamsGrs(&$d=array(), &$options = array()) {
	if (!empty($d['params'])) {
		...
		if (is_array($d['params'])) {
			$params = utilsGrs::jsonEncode($d['params']);
	...
	if(isset($d['code'])) {
		if ($d['code'] == '') {
			$d['code'] = prepareFieldCodeGrs($d['label']).'_'.rand(0, 9999999);
		}
	}
	return $d;
}
function prepareFieldCodeGrs($string) {   
 ...
 */
function recImplodeGrs($glue, $array) {
	$res = '';
	$i = 0;
	$count = count($array);
	foreach($array as $el) {
...
function objectToArrayGrs($data) {
	...
	foreach ($data as $key => $value) {
		...
		if (is_array($value)) 
			$result[$key] = objectToArrayGrs($value);
		else
			$result[$key] = $value;
	}
	return $result;
 ...
 */
function multArrayMapGrs($callback, $array) {
	if(is_array($array)) {
		foreach($array as $k => $v) {
			if(is_array($v)) {
				$array[ $k ] = multArrayMapGrs($callback, $v);
			} else {
