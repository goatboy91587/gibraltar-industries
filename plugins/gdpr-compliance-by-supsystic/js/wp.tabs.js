	...
	var methods = {
		init : function( options ) {
			return this.each(function(){
				...
				if (!$this.hasClass('grsWpTabs')) {
					$this.addClass('grsWpTabs');
				...
				if($this.find(selector).length) {
					...
					if(!this._firstInit) {
						if(this._options.uniqId)
							$this.find('.grsTabContent').attr('data-tabs-for', this._options.uniqId);
						this._firstInit = 1;
					}
					var allTabsContent = this._options.uniqId 
						? $this.find('.grsTabContent[data-tabs-for="'+ this._options.uniqId + '"]')
						: $this.find('.grsTabContent');
					allTabsContent.hide();
