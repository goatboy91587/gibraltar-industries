<?php
class optionsViewGrs extends viewGrs {
	private $_news = array();
	public function getNewFeatures() {
		$res = array();
		$readmePath = GRS_DIR. 'readme.txt';
		if(file_exists($readmePath)) {
			$readmeContent = @file_get_contents($readmePath);
			if(!empty($readmeContent)) {
				$matchedData = '';
				if(preg_match('/= '. GRS_VERSION. ' =(.+)=.+=/isU', $readmeContent, $matches)) {
					$matchedData = $matches[1];
				} elseif(preg_match('/= '. GRS_VERSION. ' =(.+)/is', $readmeContent, $matches)) {
					$matchedData = $matches[1];
				}
    ...
    public function getAdminPage() {
		...
		if(isset($tabs[ $activeTab ]) && isset($tabs[ $activeTab ]['callback'])) {
			//frameGrs::_()->getModule('supsystic_promo')->getModel()->saveUsageStat('tab.'. $activeTab);
		...
		frameGrs::_()->addJSVar('adminOptionsGrs', 'ppsActiveTab', $activeTab);
		...
		$this->assign('breadcrumbs', frameGrs::_()->getModule('admin_nav')->getView()->getBreadcrumbs());
		//$this->assign('mainLink', frameGrs::_()->getModule('supsystic_promo')->getMainLink());
	...
	public function getTabContent() {
		frameGrs::_()->addScript('admin.mainoptions', $this->getModule()->getModPath(). 'js/admin.mainoptions.js');
	...
	public function getSettingsTabContent() {
		frameGrs::_()->addScript('admin.settings', $this->getModule()->getModPath(). 'js/admin.settings.js');
		frameGrs::_()->getModule('templates')->loadJqueryUi();
		
		$options = frameGrs::_()->getModule('options')->getAll();
		$this->assign('options', $options);
		$this->assign('exportAllSubscribersUrl', uriGrs::mod('subscribe', 'getWpCsvList'));
