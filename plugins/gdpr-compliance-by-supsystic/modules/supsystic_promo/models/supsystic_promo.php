<?php
class supsystic_promoModelGrs extends modelGrs {
	private $_apiUrl = '';
	...
	public function saveUsageStat($code, $unique = false) {
		...
		return dbGrs::query($query);
	}
	private function _checkUniqueStat($code) {
		$uniqueStats = get_option(GRS_CODE. '_unique_stats');
		...
		$uniqueStats[] = $code;
		update_option(GRS_CODE. '_unique_stats', $uniqueStats);
		return false;
	}
	...
	public function getUserStatsCount() {
		...
		return (int) dbGrs::get($query, 'one');
	}
