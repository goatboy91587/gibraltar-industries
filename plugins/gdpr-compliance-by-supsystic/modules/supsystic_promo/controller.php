<?php
class supsystic_promoControllerGrs extends controllerGrs {
    public function welcomePageSaveInfo() {
		$res = new responseGrs();
		installerGrs::setUsed();
		if($this->getModel()->welcomePageSaveInfo(reqGrs::get('get'))) {
			$res->addMessage(__('Information was saved. Thank you!', GRS_LANG_CODE));
		} else {
			$res->pushError($this->getModel()->getErrors());
		}
		$originalPage = reqGrs::getVar('original_page');
		$http = isset($_SERVER['HTTPS']) && !empty($_SERVER['HTTPS']) ? 'https://' : 'http://';
		if(strpos($originalPage, $http. $_SERVER['HTTP_HOST']) !== 0) {
			$originalPage = '';
		}
		redirectGrs($originalPage);
	}
	public function sendContact() {
		$res = new responseGrs();
		$time = time();
		$prevSendTime = (int) get_option(GRS_CODE. '_last__time_contact_send');
			...
			if($validate) {
				...
				foreach($validate as $v) {
					...
					switch($v) {
						case 'notEmpty':
							if(empty($data[ $fName ])) {
								$error = $fData['html'] == 'selectbox' ? __('Please select %s', GRS_LANG_CODE) : __('Please enter %s', GRS_LANG_CODE);
								$error = sprintf($error, $fData['label']);
							}
							break;
						case 'email':
							if(!is_email($data[ $fName ])) 
								$error = __('Please enter valid email address', GRS_LANG_CODE);
							break;
					}
					if(!empty($error)) {
		...
		if(!$res->error()) {
			...
			$msg .= 'Plugin: '. GRS_WP_PLUGIN_NAME. '<br />';
			foreach($fields as $fName => $fData) {
			...
			if(frameGrs::_()->getModule('mail')->send('support@supsystic.zendesk.com', $data['subject'], $msg, $data['name'], $data['email'])) {
				update_option(GRS_CODE. '_last__time_contact_send', $time);
			} else {
				$res->pushError( frameGrs::_()->getModule('mail')->getMailErrors() );
			}
	...
	public function addNoticeAction() {
		$res = new responseGrs();
		$code = reqGrs::getVar('code', 'post');
		$choice = reqGrs::getVar('choice', 'post');
		if(!empty($code) && !empty($choice)) {
			$optModel = frameGrs::_()->getModule('options')->getModel();
			switch($choice) {
				case 'hide':
	...
	public function getPermissions() {
		return array(
			GRS_USERLEVELS => array(
				GRS_ADMIN => array('welcomePageSaveInfo', 'sendContact', 'addNoticeAction'),
			),
		);
	}
}