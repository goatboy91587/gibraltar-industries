...
class utilsGrs {
    static public function jsonEncode($arr) {
        return (is_array($arr) || is_object($arr)) ? json_encode_utf_normalGrs($arr) : json_encode_utf_normalGrs(array());
    }
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsGrs::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsGrs::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameGrs::_()->getModule($m)) 
                return false;
            if(!is_dir(frameGrs::_()->getModule($m)->getModDir())) 
                return false;
        }
        return true;*/
        foreach(frameGrs::_()->getModules() as $m) {
	...
	static public function isPluginsPage() {
		return (basename(reqGrs::getVar('SCRIPT_NAME', 'server')) === 'plugins.php');
	}
	...
	/**
	 * Parse worgrsess post/page/custom post type content for images and return it's IDs if there are images
	...
	static public function getReferalUrl() {
		// Simple for now
		return reqGrs::getVar('HTTP_REFERER', 'server');
	}
