...
class responseGrs {
    ...
    public function ajaxExec($forceAjax = false) {
        $isAjax = $this->isAjax();
        $redirect = reqGrs::getVar('redirect');
        if(count($this->errors) > 0)
            $this->error = true;
        if($isAjax || $forceAjax)
            exit( json_encode_utf_normalGrs($this) );
        /*if($redirect)
            redirectGrs($redirect);*/
        return $this;
    }
	public function mainRedirect($redirectUrl = '') {
		$redirectUrl = empty($redirectUrl) ? GRS_SITE_URL : $redirectUrl;
		$redirectData = array();
		if(!empty($this->errors)) {
			$redirectData['grsErrors'] = $this->errors;
		}
		if(!empty($this->messages)) {
			$redirectData['grsMsgs'] = $this->messages;
		}
        return redirectGrs($redirectUrl. (strpos($redirectUrl, '?') ? '&' : '?'). http_build_query($redirectData));
	}
    ...
    /**
     * Alias for responseGrs::addError, @see addError method
     */
