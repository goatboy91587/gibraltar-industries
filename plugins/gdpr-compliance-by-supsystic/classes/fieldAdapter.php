...
class fieldAdapterGrs {
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapterGrs', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['optionsGrs'] = array();
    ...
    static public function userFieldDestToDB($value) {
        return utilsGrs::jsonEncode($value);
    }
    static public function userFieldDestFromDB($value) {
        return utilsGrs::jsonDecode($value);
    }
    static public function taxDataHtml($field) {
        ...
        $categories = frameGrs::_()->getModule('products')->getCategories();
        $brands = frameGrs::_()->getModule('products')->getBrands();
        $cOptions = array();
        $bOptions = array();
        if(!empty($categories)) {
        ...
        return '<div>'. __('Apply To', GRS_LANG_CODE). '
            <div id="tax_address">
                <b>'. __('Address', GRS_LANG_CODE). '</b><br />
                '. __('Destination', GRS_LANG_CODE). ':'. htmlGrs::checkboxlist('params[dest]', array('optionsGrs' => $listOfDest)). '<br />
                '. __('Country', GRS_LANG_CODE). ':'. htmlGrs::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. __('Categories', GRS_LANG_CODE). '</b><br />
                '. (empty($cOptions) ? __('You have no categories', GRS_LANG_CODE) : htmlGrs::checkboxlist('params[categories][]', array('optionsGrs' => $cOptions))). '<br />
                    <b>'. __('Brands', GRS_LANG_CODE). '</b><br />
                '. (empty($bOptions) ? __('You have no brands', GRS_LANG_CODE) : htmlGrs::checkboxlist('params[brands][]', array('optionsGrs' => $bOptions))). '<br />
            </div>
            <div>'. __('Tax Rate', GRS_LANG_CODE).': '. htmlGrs::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. __('Absolute', GRS_LANG_CODE).': '. htmlGrs::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? __('Not selected', GRS_LANG_CODE) : __($notSelected);
			}
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = frameGrs::_()->getModule('optionsGrs')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                $output .= __($key).':<br />';
                $output .= htmlGrs::text('params[attr]['.$key.']',array('value'=>$value)).'<br />';
            }
        } else {
                $output .= __('class', GRS_LANG_CODE).':<br />';
                $output .= htmlGrs::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= __('id', GRS_LANG_CODE).':<br />';
                $output .= htmlGrs::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['optionsGrs']))
            return;
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsGrs'][0] = in_array(0,$field->value['categories'])?__('Deselect All'):__('Select All', GRS_LANG_CODE);
            foreach($categories as $c) {
                $field->htmlParams['optionsGrs'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = frameGrs::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = frameGrs::_()->getTable('states')
				->leftJoin( frameGrs::_()->getTable('countries'), 'country_id' )
	...
	static public function getFontsList() {
		return array("Abel", "Abril Fatface", "Aclonica", "Acme", "Actor", "Adamina", "Advent Pro",
			...
			"Butterfly Kids", "Cabin", "Cabin Condensed", "Cabin Sketch", "Caesar Dressing", "Cagliostro", "Calligragrstti",
