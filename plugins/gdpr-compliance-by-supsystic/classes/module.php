<?php
abstract class moduleGrs extends baseObjectGrs {
	...
	public function __construct($d/*, $params = array()*/) {
		...
		if(isset($d['ex_plug_dir']) && !empty($d['ex_plug_dir'])) {
			...
			$this->setExternalDir( utilsGrs::getExtModDir($d['ex_plug_dir']) );
			$this->setExternalPath( utilsGrs::getExtModPath($d['ex_plug_dir']) );
		}
	}
	...
	public function getModDir() {
		if(empty($this->_externalDir)) {
			return GRS_MODULES_DIR. $this->getCode(). DS;
		} else {
	...
	public function getModPath() {
		if(empty($this->_externalPath)) {
			return GRS_MODULES_PATH. $this->getCode(). '/';
		} else {
	...
	protected function _createController() {
		...
		if(file_exists($this->getModDir(). 'controller.php')) {
			$className = '';
			if(importGrs($this->getModDir(). 'controller.php')) {
				$className = toeGetClassNameGrs($this->getCode(). 'Controller');
			}
			if(!empty($className)) {
	...
	protected function _createHelper() {
		...
		if (file_exists($this->getModDir().'helper.php')) {
			...
			importClassGrs($helper, $this->getModDir(). 'helper.php');
