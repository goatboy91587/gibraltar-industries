...
class fieldGrs {
	 ...
	 */
	public $adapt = array('htmlGrs' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue($value, $fromDB = false) {
		if(isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB)
			$value = fieldAdapterGrs::_($value, $this->adapt['dbFrom'], fieldAdapterGrs::DB);
		$this->value = $value;
	}
	...
	public function drawHtml($tag, $id) {
		if(method_exists('htmlGrs', $this->html)) {
			$method = $this->html;
			...
			if($this->adapt['html']) {
				fieldAdapterGrs::_($this, $this->adapt['html'], fieldAdapterGrs::HTML);
			}
			$params = $this->processParams($tag, $id);
			if ($params != '')
				return $params;
			if ($this->name == 'default_value') {
				$optionsFromDb = frameGrs::_()->getModule('optionsGrs')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => __('Select', GRS_LANG_CODE));
					foreach($optionsFromDb as $k => $v)
						$options[$k] = $v;
					$method = 'selectbox';
					$this->htmlParams['optionsGrs'] = $options;
				}
			}
			$htmlContent = htmlGrs::$method($this->name, $this->htmlParams);
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlGrs::nameToClassId($this->name). '"></div>';
	...
	public function displayValue() {
		...
		switch($this->html) {
			case 'countryList':
				$value = fieldAdapterGrs::displayCountry($this->value);
				break;
			case 'statesList':
				$value = fieldAdapterGrs::displayState($this->value);
				break;
			case 'checkboxlist':
				$options = $this->getHtmlParam('optionsGrs');
				$value = array();
				...
				if ($this->value == '') {
					...
					if(is_array($this->value)) {
						...
						if(!empty($options) && is_array($options)) {
							...
							$value = recImplodeGrs('<br />', $valArr);
						} else {
							$value = recImplodeGrs('<br />', $this->value);
						}
					} else
						$value = $this->value;
				}
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = frameGrs::_()->getModule('optionsGrs')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsGrs'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlGrs', $method)) {
			$output .= htmlGrs::$method($name, $params);
			$output .= htmlGrs::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams($tag, $id){
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = __('Add Checkbox', GRS_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = __('Add Item', GRS_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = __('Add Radio Button', GRS_LANG_CODE);
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterGrs::_($id,'getExtraFieldOptions',fieldAdapterGrs::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"'.$image_tag.'>'.__('Dimensions', GRS_LANG_CODE).':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= __('width', GRS_LANG_CODE).':<br />';
					$output .= htmlGrs::text('params[width]',array('value'=>$width)).'<br />';
					$output .= __('height', GRS_LANG_CODE).':<br />';
					$output .= htmlGrs::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterGrs::_($this, $this->adapt['htmlParams'], fieldAdapterGrs::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">'.__('Click to set field "id" and "class"', GRS_LANG_CODE).'</a>';
				$output .= '<div class="attributes" style="display:none;">'.__('Attributes', GRS_LANG_CODE).':<br />';
				$output .= fieldAdapterGrs::_($params,'getFieldAttributes',  fieldAdapterGrs::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	function checkVarFromParam($param, $element) {
		return utilsGrs::xmlAttrToStr($param, $element);
	  ...
	  // load xml structure of parameters
	   ...
	   foreach ($config->params->param as $param) {
		 // read the variables
		  ...
		  $helper = $this->checkVarFromParam($param,'helperGrs');
		  $module = $this->checkVarFromParam($param,'moduleGrs');
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!frameGrs::_()->getModule($module)) 
			return false; 
		// check for xml file with params structure  
	   if(frameGrs::_()->getModule($module)->isExternal())
		   $config_xml = frameGrs::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = GRS_MODULES_DIR.$module.DS.'mod.xml';

	   if (!file_exists($config_xml)) {
		   // if there is no configuration file for this $module
		   return __('There are no configuration options for this module', GRS_LANG_CODE);
	   }
	   $output = '';
	   ...
	   elseif(is_object($this->value) || is_array($this->value))
			$params = objectToArrayGrs($this->value);
	   //if (!empty($params)) {
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // if helper is needed to render the object
					  ...
					  $helper = frameGrs::_()->getModule($hmodule)->getHelper();
					  if ($helper) {
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsGrs'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;
				  ...
				  // output label and html element
					 ...
					 $output .= '</label><br />';
					 $output .= htmlGrs::$method($name,$htmlParams).'<br />';
					 $output .= '</div>';
			   }
			   $i++;
			 $output .= '</div>';
