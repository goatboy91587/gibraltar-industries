...
class MTSNB_Shared {
	...
	public function get_notification_bar_data() {
		...
		foreach( $all_bars as $post ) :
			...
			if ( $passed_location_conditions && $passed_time_conditions && $passed_mobile_conditions && $passed_logged_conditions && $passed_less_conditions && $passed_referrer_conditions && $passed_utm_conditions && $passed_after_conditions ) {
				$current_post_data[$bar_id] = $meta_values;
				$current_post_data[$bar_id]['priority'] = isset($meta_values['priority']) ? $meta_values['priority'] : 'medium';
			}

		endforeach; wp_reset_postdata();

		if(!empty($current_post_data)) {
			$found = false;
			foreach($current_post_data as $current_post_id => $data) {
				if( 'high' === $data['priority']) {
					$this->bar_id   = $current_post_id;
					$this->bar_data   = $data;
					$found = true;
				} else if( 'medium' === $data['priority'] && !$found ) {
					$this->bar_id   = $current_post_id;
					$this->bar_data   = $data;
					$found = true;
				} else if(!$found) {
	...
	public function bar_output( $bar_id, $meta_values ) {
		...
		$align = isset( $meta_values['align'] ) && !empty( $meta_values['align'] ) ? $meta_values['align'] : 'center';
		...
		$menu_selector = isset($meta_values['slidein_selector']) ? $meta_values['slidein_selector'] : '';
		$mtsnb_sp_display = isset($meta_values['slidein_display']) ? $meta_values['slidein_display'] : '';

		$content_type = isset($meta_values['content_type']) ? $meta_values['content_type'] : '';
		...
		?>
		<?php if($meta_values['content_type'] === 'slidein-posts') { ?>
				<div class="mtsnb-sp-mask"></div>
			<?php if(empty($menu_selector) && !is_admin()) {
				$mtsnb_sp_button_pos = isset($meta_values['slidein_button_position']) ? $meta_values['slidein_button_position'] : 'bottom-right';
			?>
					<div id="mtsnb-sp-selector" class="<?php echo esc_attr($mtsnb_sp_button_pos); ?>">
						<i class="fa fa-<?php echo $show_icon; ?>"></i>
						<span class="mtsnb-sp-icon"></span>
					</div>
			<?php } ?>
		<?php } ?>
		<div class="mtsnb mtsnb-sp-<?php echo $mtsnb_sp_display; ?> mtsnb-<?php echo $meta_values['content_type'] ?> <?php echo $state_class.$visibility_class.$remember_state_class.$screen_position_class.$css_position_class; ?>" id="mtsnb-<?php echo $bar_id; ?>" data-mtsnb-id="<?php echo $bar_id; ?>" style="<?php echo $style;?>" data-bar-animation="<?php echo $animation; ?>" data-bar-content-animation="<?php echo $content_animation; ?>" data-sp-selector= "<?php echo $menu_selector; ?>">
			<style type="text/css">
				...
				.mtsnb .mtsnb-button, #mtsnb-sp-selector, #mtsnb-sp-selector .mtsnb-sp-icon { color: <?php echo $button_color;?>!important; background-color: <?php echo $button_bg_color;?>;}
			</style>
			<div class="mtsnb-container-outer">
				<div class="mtsnb-container mtsnb-clearfix" style="padding-top: <?php echo $padding; ?>px; padding-bottom: <?php echo $padding; ?>px; text-align: <?php echo $align; ?>">
					<?php do_action('before_mtsnb_content'); ?>
					<?php $this->bar_content( $meta_values ); ?>
					<?php do_action('after_mtsnb_content'); ?>
				</div>
				<?php if ( 'no_button' !== $button_type && 'slidein-posts' !== $content_type ) { ?>
				<?php if ( 'toggle_button' === $button_type ) {?>
						<a href="#" class="mtsnb-show" style="<?php echo $btn_style; ?> padding: <?php echo $padding; ?>px;"><?php echo $button_open_icon; ?></a>
					<?php } ?>
	...
	public function bar_content( $options ) {
		...
		switch ( $options[$b_prefix.'content_type'] ) {
			...
			break;
			case 'slidein-posts':
			case 'slidein-posts-b':
				$text          = wp_kses_post( $options[$b_prefix.'slidein_text'] );
				$posts_number  = intval( $options[$b_prefix.'slidein_number'] );
				$posts_to_show = $options[$b_prefix.'slidein_to_show'];
				$post_thumb    = (bool) isset( $options[$b_prefix.'slidein_post_thumb'] );
				$post_date    = (bool) isset( $options[$b_prefix.'slidein_post_date'] );
				$post_excerpt    = (bool) isset( $options[$b_prefix.'slidein_post_excerpt'] );
				$new_label    = (bool) isset( $options[$b_prefix.'slidein_new_label'] );
				$close_icon = (isset($options['close_icon']) && !empty($options['close_icon'])) ? $options['close_icon'] : 'times';
				$data = array();
				if ( !empty( $text ) )
					echo '<span class="mtsnb-text">'
							.$text.
							'<i class="mtsnb-close-sp fa fa-'.$close_icon.'"></i></span>';

				if ( 1 < $posts_number ) {
					?>
					<div class="mtsnb-sp-container">
					<?php
				} else {
					?>
					<div class="mtsnb-posts">
				...
				if($posts_to_show === 'feed') {
					$url = trim($options['slidein_feed']);
					...
					if ($feed->error()) {
						...
						foreach($feed->get_items(0, $posts_number) as $item) {
							$data[] = array(
								'id' => $item->get_id(true),
				...
				} elseif($posts_to_show === 'custom_content') {
					$custom_content = isset($options['sp']) ? $options['sp'] : array();
					if(!empty($custom_content)) {
						foreach($custom_content as $content) {
							$target = isset($content['target']) ? $content['target'] : '';
							$data[] = array(
								'id' => '',
								'image' => $content['image'],
								'title' => $content['title'],
					...
					$post_type = (isset($options['slidein_custom_post_types']) && !empty($options['slidein_custom_post_types'])) ? $options['slidein_custom_post_types'] : 'post';
					$post_authors = (isset($options['slidein_post_authors']) && !empty($options['slidein_post_authors'])) ? $options['slidein_post_authors'] : '';
					$meta_key = (isset($options['slidein_post_meta_key']) && !empty($options['slidein_post_meta_key'])) ? $options['slidein_post_meta_key'] : '';
					$meta_value = (isset($options['slidein_post_meta_value']) && !empty($options['slidein_post_meta_value'])) ? $options['slidein_post_meta_value'] : '';
					$categories = (isset($options['slidein_custom_categories']) && !empty($options['slidein_custom_categories'])) ? $options['slidein_custom_categories'] : '';
					$tags = (isset($options['slidein_custom_tags']) && !empty($options['slidein_custom_tags'])) ? $options['slidein_custom_tags'] : '';
					...
					if ( 'recent' !== $posts_to_show ) {
						...
						} else if('custom_post_types' === $posts_to_show) {
				...
				if(!empty($data)) {
					foreach($data as $d) {
						...
						$new_text = '';
						$visited_ids = array();
						$target = (isset($d['target']) && 'yes' === $d['target']) ? '_blank' : '';
						if(isset($_COOKIE['mtsnb_lastvisit_posts'])) {
							$visited_ids = json_decode($_COOKIE['mtsnb_lastvisit_posts']);
						}
						if($d['id'] && !in_array($d['id'], $visited_ids) && isset($_COOKIE['mtsnb_lastvisited'])) {
							$lastvisited = $_COOKIE['mtsnb_lastvisited'];
							$post_time = get_post_time('U', false, $d['id']);
							if($post_time >= $lastvisited) {
								$attr = 'data-new';
								if($new_label) {
									$new_text = __('New', $this->plugin_name);
								}
							}
						}
						?>
						<div class="mtsnb-post" data-post-id="<?php echo $d['id']; ?>" <?php echo $attr; ?>>
								<div class="mtsnb-inner-wrapper">
									<div class="mtsnb-meta-data">
										<?php if($new_text) { ?>
											<span class="mtsnb-new-label"><?php echo $new_text; ?></span>
										<?php } ?>
										<?php if($post_date && $d['date']) { ?>
											<span class="mtsnb-date"><?php echo $d['date']; ?></span>
										<?php } ?>
										<div class="mtsnb-post-title">
											<a rel="nofollow" href="<?php echo esc_url( $d['link'] ); ?>" target="<?php echo $target; ?>" title="<?php echo esc_attr( $d['title'] ); ?>"><?php echo esc_html( $d['title'] ); ?></a>
										</div>
									</div>
								<?php if ( $d['image'] && $post_thumb ) {?>
								<div class="mtsnb-post-img">
									<a rel="nofollow" href="<?php echo esc_url( $d['link'] ); ?>" target="<?php echo $target; ?>" title="<?php echo esc_attr( $d['title'] ); ?>">
										<img src="<?php echo esc_url($d['image']); ?>" >
									</a>
								</div>
								<?php } ?>
								<?php if($d['content'] && $post_excerpt) { ?>
								<div class="mtsnb-post-data">
									<?php echo $d['content']; ?>
								</div>
								<?php } ?>
			...
			case 'newsletter-b':
				...
				$consent		= $options[$b_prefix.'newsletter_consent'];
				...
				$consent		= isset($options[$b_prefix.'newsletter_consent']) ? $options[$b_prefix.'newsletter_consent'] : '';
					...
					<form class="mtsnb-fb-form" method="post" action="https://feedburner.google.com/fb/a/mailverify" target="mtsnbwindow">
						...
						<?php if($consent) { ?>
								<div class="mtsnb-consent-wrapper">
									...
									<label for="mtsnb-consent-field"><?php echo esc_html($consent); ?></label>
								</div>
							<?php } ?>
	...
	/**
	 * Notification bar remove impressions.
	 *
	 * @since    1.0.0
	 */
	public function remove_impressions() {
		$reset_op = $_POST['reset_op'];
		$id = $_POST['id'];
		$opt_array = get_option('mtsnb_stats');

		if($reset_op === 'reset-impressions') {
			if($id === 'all') {
				foreach($opt_array as $key => $opt) {
					$opt_array[$key]['impressions'] = 0;
				}
			} else {
				$opt_array[$id]['impressions'] = 0;
			}
		} else if($reset_op === 'reset-clicks') {
			if($id === 'all') {
				foreach($opt_array as $key => $opt) {
					$opt_array[$key]['clicks'] = 0;
				}
			} else {
				$opt_array[$id]['clicks'] = 0;
			}
		}
				...
				if ( !empty( $custom_urls ) ) {
					...
					foreach ( $custom_urls as $custom_url ) {
						...
						if ( trailingslashit( $current_url ) === trailingslashit( $custom_url ) ||
							 ...
							 strpos( $current_url_encoded, $custom_url ) ) {
							return true;
						}
					}
...
add_action('wp', function(){
	if(is_singular()) {
		global $post;
		$visited_ids = array();
		if(isset($_COOKIE['mtsnb_lastvisit_posts']) && !empty($_COOKIE['mtsnb_lastvisit_posts'])) {
			$visited_ids = json_decode($_COOKIE['mtsnb_lastvisit_posts']);
		}
		if(!in_array($post->ID, $visited_ids)) {
			$visited_ids[] = $post->ID;
		}
		setcookie( 'mtsnb_lastvisit_posts', json_encode($visited_ids), time() + (10 * 365 * 24 * 60 * 60), '/' );
